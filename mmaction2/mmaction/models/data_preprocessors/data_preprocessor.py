...
class ActionDataPreprocessor(BaseDataPreprocessor):
    ...
    def preprocess(self,
        ...
        if inputs and isinstance(inputs[0], (list, tuple)):
            # Handle multi-view inputs like [[v1, v2], ...]
            num_views = len(inputs[0])
            assert all(len(x) == num_views for x in inputs), 'Inconsistent number of views'
            view_batches = []
            for i in range(num_views):
                view_inputs = [x[i] for x in inputs]
                view_batches.append(stack_batch(view_inputs))
            batch_inputs = torch.stack(view_batches, dim=1)
        else:
