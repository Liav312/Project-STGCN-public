...
class Graph:
    ...
    def __init__(self,
        ...
        if isinstance(layout, dict):
            ...
            if layout not in ['openpose', 'nturgb+d', 'coco']:
                custom_cls = MODELS.get(layout)
                if custom_cls is not None:
                    custom_graph = custom_cls(layout=layout, mode=mode)
                    self.__dict__.update(custom_graph.__dict__)
                    return
                raise AssertionError(f'Layout {layout} not supported')
