...
class BaseHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        # ``loss_cls`` might be ``None`` when the head is used only for
        # feature extraction (e.g. projection heads in contrastive
        # learning). In such cases we should skip building the loss
        # module and leave ``self.loss_cls`` as ``None``.
    ...
    def loss_by_feat(self, cls_scores: torch.Tensor,
        ...
        if self.loss_cls is None:
            raise NotImplementedError(
                '`loss_cls` is None, `loss_by_feat` should be implemented in '
                'subclasses when no classification loss is required.')
        loss_cls = self.loss_cls(cls_scores, labels)
        # ``loss_cls`` may be a dictionary or a single tensor
