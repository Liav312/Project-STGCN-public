...
class RecognizerGCN(BaseRecognizer):
    ...
    def extract_feat(self,
        ...
        # `FormatGCNInput` may insert a dummy clip dimension when the
        # dataset already provides single-clip tensors. In contrastive
        # training where we treat different augmented views as clips, this
        # leads to an extra dimension of size 1 before the temporal axis.
        # Remove it so that the STGCN backbone receives tensors shaped
        # ``(N, M, T, V, C)`` as expected.
        if inputs.dim() == 6 and inputs.size(2) == 1:
